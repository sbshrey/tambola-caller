...
## V1.5 English, Hindi and Hinglish — 2026-09-24

- Added a persistent Call language selector and a Try voice sample (22, without drawing). All 90 numbers have sayings in each language. Replaced the 47 plain English scripts while retaining the existing 43 recordings. Hindi uses Devanagari sayings and number names; Hinglish uses Hindi sayings plus English digits and number names, with Roman Hindi on screen. References and adaptations are documented in `docs/CALLS.md`; there is no claim of a universal Hindi calling standard.
- The selected language follows automatic playback, repeat, fallback speech, MP3 downloads/shares, number-message text, and number words on PNGs. Changing it preserves calls, players, schemes and awards, and invalidates previously prepared files. Rapid changes cannot leave a stale-language MP3 or image ready to share. Existing saved games default to English; new rounds retain the chosen language. Interface and winner-management controls remain in English.
- Generated 90 Hindi and 90 Hinglish MP3s plus 47 English replacements, using the existing female-host `coral` voice and language-specific prompts. The three manifests bind every final file to its current script, settings and SHA-256. Total pack size is approximately 24 MB. Generation and the optional transcription audit use a local environment key; the runtime and build contain no key and never call OpenAI.
- Automated transcription without the expected wording checked all 227 new/replaced files. The current per-language reports are `audio-validation-v1.5-en.json` (47), `audio-validation-v1.5-hi.json` (90), and `audio-validation-v1.5-hinglish.json` (90); all identify the expected number and match final file hashes. Some initial clips omitted or blurred their full-number ending: English 29/80, Hindi 15/28/37/39/42/48/58/63/67/73/80/87, and Hinglish 33/36/44/65/87 were regenerated with a fuller ending sentence and checked again. Hindi 58 accepts the documented spelling variant [अठावन](https://hindwidictionary.com/meaning-of-athaavan). Secondary transcription models resolved several script/format ambiguities. These are number checks, not proof of every phrase, accent or native-speaker preference. The original `audio-validation.json` remains a historical v1.2 report.
- `npm test`: **66 passed**. Coverage includes all 270 assets/scripts/hashes, language persistence and legacy saves, language-specific URLs/files, fallback speech, stale playback/preparation, privacy, PNG invalidation, generator selection, and the existing game/prize/sharing checks. `npm run build` and `git diff --check` pass. The v1.5.0 worker includes all language modules and 270 clips; its existing byte-range behavior is covered by the worker fixture.
- A jsdom integration fixture (`scripts/check-language-dom.mjs`, optional external jsdom dependency) exercised the actual HTML and application module: selector changes, displayed phrase and number words, repeat/preview URL selection, ready MP3 sharing, downloads, saved preference, rapid changes, and unchanged game/player data. It passed. Media playback, canvas, and fetch were stubbed; this is not a browser/device check.
- The in-app browser connection was unavailable in this session (authentication-method error), so no new visual layout, real audio playback, or physical phone validation is claimed. Prior v1.4 browser evidence above does not establish the new language controls or Hindi canvas rendering. Native-speaker listening and Android/iPhone WhatsApp handoff remain to be checked. No WhatsApp message was sent.
- Existing offline users must open online, close all Tambola tabs, and reopen for the waiting v1.5 worker to activate. The footer confirms **v1.5 · All 3 languages ready offline** only after the active worker confirms 270 cached clips. The update preserves saved progress and does not replace an active game midway.

## V1.6 separate PNG sharing — 2026-09-24

- Added a 46 × 46 CSS-pixel share icon in the top-right header of the current number, recent calls and board sections. Each has a specific accessible name and shares only its section. The previous combined image remains as **Share full summary**, with Preview. Recent View all moves to the helper row to keep the header uncluttered on narrow screens.
- The number PNG is 1080 × 1080 and includes the chosen language's number words and saying. Recent calls use a 1080 × 720 image, with the latest ten calls in a five-column/two-row grid, latest first. The board uses 1080 × 1140, with all 90 cells and distinct called/latest marks. Combined summary remains 1080 × 1540. Each file has a distinct format/call/number filename. No player or winner details enter the renderer.
- All formats reuse canvas primitives, background preparation, the existing native-file sharing helper, and a single preview/download dialog. Images are prepared before taps. Unsupported sharing opens the selected format's preview; cancellation stays quiet. Overlapping shares are blocked. Undo, language changes and reset invalidate all prepared images and close stale previews. Late native-share failures cannot reopen an older snapshot. Preview object URLs are revoked on close or invalidation.
- **69 Node tests passed**, including requested-content isolation, dimensions/filenames, Hindi number/saying text, first-call placeholders, and stale-result handling across all three separate formats. The service-worker fixture checks the new image UI module under v1.6.0 and the unchanged 270 voice clips. Static build and diff checks pass.
- The optional jsdom integration fixture now exercises all four share buttons against the actual app module and a native canvas adapter. It verifies immediate file handoff before yielding from the click, actual PNG signatures/dimensions, no game change from sharing, overlapping-share prevention, quiet cancellation, the correct fallback title/download, preview URL cleanup, undo, language switching, stale native failures and new-game reset. It passed. Native sharing/media/fetch remain simulated; no WhatsApp message was sent.
- Rendered and visually inspected PNG samples using `@napi-rs/canvas` outside production dependencies: English/Hindi/Hinglish current numbers, recent calls, board, combined summary, a first-call recent list and completed 90-number board. The initial Hindi render revealed missing glyphs with Arial; the renderer now explicitly selects installed Devanagari font families. The corrected Hindi output was inspected and is readable. No font is downloaded by the app.
- The browser connection still reports an unsupported authentication method, so these checks do not establish mobile browser layout or actual Android/iPhone WhatsApp delivery. The local canvas exports and DOM behavior were checked separately. Existing users must close old tabs and reopen online to activate v1.6; saved games and language preferences remain intact.
