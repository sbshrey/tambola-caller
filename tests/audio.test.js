...
test('file sharing preserves the tap activation and does not include private game details', async () => {
  ...
  let called = false;
  const promise = shareFile(file, { canShare: (data) => data.files[0] === file, share(data) {
...
test('file sharing distinguishes cancellation from unsupported or failed handoff', async () => {
  ...
  assert.equal(await shareFile(file, {}), 'fallback');
  assert.equal(await shareFile(file, { canShare: () => false, share: assert.fail }), 'fallback');
  assert.equal(await shareFile(file, { canShare: () => true, share() { throw { name: 'AbortError' }; } }), 'cancelled');
  assert.equal(await shareFile(file, { canShare: () => true, share() { throw { name: 'NotAllowedError' }; } }), 'fallback');
});
