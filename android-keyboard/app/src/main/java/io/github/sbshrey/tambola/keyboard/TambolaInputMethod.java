...
public final class TambolaInputMethod extends InputMethodService {
    ...
    private boolean practice, textField, mediaBusy;
    private long lastDraw;
    private int session;
    private String panel = "Call", awardId;

    @Override public void onCreate() { setTheme(R.style.AppTheme); super.onCreate(); preferences = new Preferences(this); audio = new CallAudio(this); }
    ...
    @Override public View onCreateInputView() {
        ...
        store = new GameStore(this, practice); render(); return root;
    }
    @Override public void onStartInputView(EditorInfo info, boolean restarting) {
        super.onStartInputView(info, restarting); session++; mediaBusy = false; panel = "Call";
        practice = getPackageName().equals(info.packageName) && PRACTICE.equals(info.privateImeOptions); store = new GameStore(this, practice);
    ...
    private void board(Game game) {
        ...
        Ui.action(body, "Undo / New game", Ui.PAPER, () -> open(practice ? "practice" : "board"));
    }
    private void voice() {
    ...
    private InputConnection emptyEditor() {
        ...
        if (before.length() > 0 || after.length() > 0 || (selected != null && selected.length() > 0)) { say("Send or clear the current message first."); return null; } return editor;
    }
